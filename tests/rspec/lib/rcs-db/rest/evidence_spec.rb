  ...
  describe EvidenceController do
    ...
    describe '#index' do
      ...
      def index_with_params params
        ...
        cnt = result.count
        cnt = -1 if cnt == 0
        expect(cnt).to eql subject.count
      ...
      context 'when "info" is provided' do
        ...
        it 'return the matching evidences (search using $near)' do
          criteria = index_with_params 'filter' => common_filters.merge("info" => ["lon:9.1915256,lat:45.4766561,r:500"])
          expect(criteria.first).to eql evidence4

          criteria = index_with_params 'filter' => common_filters.merge("info" => ["lon:9.1915256,lat:45.4766561,r:500"])
          expect(criteria.first).to eql evidence4

          criteria = index_with_params 'filter' => common_filters.merge("info" => ["lon:9.19,lat:45.47,r:100"])
          expect(criteria.first).to be_nil

          criteria = index_with_params 'filter' => common_filters.merge("info" => ["lon:9.19,lat:45.47,r:1000"])
          expect(criteria.first).to eql evidence4

          criteria = index_with_params 'filter' => common_filters.merge("info" => ["lon:30.0,lat:30.0,r:5000"])
          expect(criteria.first).to be_nil
        end
      end
