...
class EvidenceFilter
  ...
  def self.create_default
    ...
    EvidenceFilter.destroy_all(conditions: {name: "Keylogged passwords last month"})
    ...
    EvidenceFilter.destroy_all(conditions: {name: "Facebook chats last week"})
    ...
    EvidenceFilter.destroy_all(conditions: {name: "Used application last 24h"})
    ...
    EvidenceFilter.destroy_all(conditions: {name: "Opened files last 24h"})
    ...
    EvidenceFilter.destroy_all(conditions: {name: "Captured files last 24h"})
    ...
    EvidenceFilter.destroy_all(conditions: {name: "Relevant evidence all time"})
    EvidenceFilter.find_or_create_by({user: [], name: "Relevant evidence all time", filter: {from: 0, to: 0, date: 'da', rel: [3,4]}.to_json})

    EvidenceFilter.destroy_all(conditions: {name: "Future calendar events"})
